...
class TestMlpSyncHybridTargetVerify(CustomTestCase):
    def test_verify_batch_keeps_its_verify_layout(self):
        """TARGET_VERIFY counts as extend, so in a DP verify step that pads to
        MAX_LEN (hybrid-SSM models do whenever a rank is idle) the MLP-sync
        extend conversion used to rewrite a hybrid model's verify rows as
        one-token extend requests, dropping the draft tokens. The batch must
        stay a verify batch of one request per draft width."""
