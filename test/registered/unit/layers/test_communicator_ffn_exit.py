...
class TestFfnExit(CustomTestCase):
    ...
    def test_finish_applies_the_decision_the_ffn_saw(self):
        """finish() must apply the decision published to the FFN even if the
        decision methods would now answer differently. Otherwise an FFN that
        skipped its all-reduce gets an output that is neither left to the next
        layer nor postprocessed, or a reduced output is reduced again."""
