...
class TestLastLayerCommunicator(CustomTestCase):
    def test_only_the_last_layer_is_last(self):
        """With all-reduce fusion on, a layer that is not marked last leaves its
        FFN all-reduce to the next layer's residual add + norm. The model's last
        layer has no next layer, so leaving it unmarked makes the final norm run
        on TP-partial sums."""
        for case in CASES:
    ...
    def test_draft_model_layer_is_last(self):
        """A NextN / MTP draft model runs a single decoder layer, often built
        with the target's num_hidden_layers and a layer id such as 0. It must
        still be marked last, or the draft's final norm runs on TP-partial
        sums."""
