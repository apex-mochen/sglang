...
class TestNemotronAuxCapture(CustomTestCase):
    def test_capture_reduces_only_its_snapshot(self):
        """Auxiliary hidden-state snapshots must equal the full hidden state at
        each boundary without changing the model output. Under DP attention, a
        snapshot taken where a Mamba / attention layer hands its output to an
        MLP layer used to see only this rank's attention-TP partial; the first
        snapshot aliased the residual, which a later norm updates in place."""
        for pattern in ("*-", "M-", "**-", "*", "*--", "-*"):
