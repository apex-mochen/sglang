...
class TestLongcatShortcut(CustomTestCase):
    def test_shortcut_does_not_add_the_residual_again(self):
        """On the last layer with scattered MoE tokens, the shortcut's postprocess
        gathers the tokens and adds the residual it is given. The dense branch
        already carries the residual, so passing it to the shortcut too adds it
        twice to the layer output."""
        tp, rows = 2, 4
