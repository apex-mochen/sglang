...
class TestNemotronMTPReduction(CustomTestCase):
    def test_attention_partial_is_reduced_once(self):
        """Under DP attention, the MTP MoE layer receives the attention output as
        a per-rank partial. It must be summed over the attention TP group exactly
        once before the residual add; a second sum scales it by the attention TP
        size."""
        for tp in (1, 2):
