...
class Layout(msgspec.Struct, frozen=True):
    """The token axes a rank's rows are sharded over.

    Axes whose group has a single rank shard nothing and are left out, so two
    layouts are equal exactly when every rank holds the same tokens under both.
    """
