...
class LayerCommunicator:
    ...
    def ffn_exit(self, forward_batch: ForwardBatch) -> "FfnExit":
        """Decide once how this layer's FFN output reduction completes.

        Use the result as a context manager around the FFN call, then apply
        the same decision to the FFN output with ``finish``.
        """
...
# The token axes each mode's rows are sharded over. MOE_FULL gathers across the
# MoE-CP group, which spans every CP rank whenever CP is enabled.
...
class FfnExit:
    """One FFN's decision on where its output reduction completes.

    Inside the ``with`` block the decision is published to the FFN as
